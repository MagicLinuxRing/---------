...
namespace ForKids.DB.BLL
{
	...
	{
		private readonly ICHARGEBASE dal=new ForKids.DB.OleDbDAL.CHARGEBASE();
		public CHARGEBASE()
		{}
		#region  Method
		...
		/// 获得数据列表
		/// </summary>
		public List<ForKids.DB.Model.CHARGEBASE> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<ForKids.DB.Model.CHARGEBASE> DataTableToList(DataTable dt)
		{
			List<ForKids.DB.Model.CHARGEBASE> modelList = new List<ForKids.DB.Model.CHARGEBASE>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ForKids.DB.Model.CHARGEBASE model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new ForKids.DB.Model.CHARGEBASE();
					...
					if(dt.Rows[n]["CHARGEDATE"]!=null && dt.Rows[n]["CHARGEDATE"].ToString()!="")
					{
